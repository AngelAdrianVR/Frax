<template>
  <div @click="commentDropdown = false" class="relative my-1 p-2 rounded-md border border-transparent hover:border-primary transition ease-in-out duration-300 group">
        <i @click.stop="commentDropdown = !commentDropdown" class="fa-solid fa-ellipsis-vertical px-[10px] py-1 rounded-full cursor-pointer hover:bg-gray5 text-xs text-primary absolute right-1 top-2 hidden group-hover:block"></i>
        ...
        <p class="text-xs text-gray3 ml-14">{{ comment?.created_at + '  .' }}<i class="fa-regular fa-heart text-red-500 ml-3 cursor-pointer"></i> {{ 0 + '   .   ' }} <span class="cursor-pointer ml-2">Responder</span> </p>

        <!-- Dropdown de comentario -->
        <div v-if="commentDropdown" 
        class="border border-gray5 flex flex-col space-y-1 rounded-md p-1 absolute w-28 h-auto top-9 -right-4 text-xs z-50 bg-white">
            <p
            @click.stop="reportComment()"
...
emits:['delete-comment', 'report-comment'],
methods:{
    deleteComment() {
        this.$emit('delete-comment', this.comment.id);
    },
    reportComment() {
        this.$emit('report-comment', this.comment.id);
    }
}
}
</script>
