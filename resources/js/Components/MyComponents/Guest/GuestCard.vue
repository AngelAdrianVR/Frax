  ...
  <div
    ...
    <div class="px-5 py-3">
      <div class="flex items-center space-x-2 absolute top-2 right-2">
        <el-popconfirm v-if="!guest.arrived_at" confirm-button-text="Si" cancel-button-text="No" icon-color="#D90537"
          title="¿Eliminar?" @confirm="deleteGuest">
          <template #reference>
            <i class="fa-regular fa-trash-can bg-[#F2F2F2] rounded-md p-2 cursor-pointer"></i>
          </template>
        </el-popconfirm>
        <PrimaryButton @click="$inertia.get(route('guests.edit', guest.id))" v-if="!guest.arrived_at"
          class="rounded-md !py-1">Editar</PrimaryButton>
      </div>
      <!-- -----------photo--------- -->
      ...
      <div class="mt-4 grid grid-cols-2">
        <section>
          ...
          <div v-if="cardDetails && guest.vehicle_details" @click="vehicleDetails = !vehicleDetails"
            ...
            <i class="fa-solid fa-angle-up transform origin-center transition duration-200 ease-out ml-5"
              :class="{ '!rotate-180': vehicleDetails }"></i>
          </div>
        </section>
        <section>
        ...
        <div v-if="vehicleDetails && cardDetails"
          ...
          <figure v-if="guest.media_vehicle?.length > 0" class="w-48 h-32 rounded-xl mx-auto">
            <img class="object-contain bg-no-repeat h-32 rounded-lg" :src="guest.media_vehicle[0]?.original_url"
              alt="Sin imagen" />
          </figure>
    ...
    <div @click="cardDetails = !cardDetails" class="flex items-center text-primary font-bold cursor-pointer mx-5">
      <p>{{ cardDetails ? "Ocultar" : "Ver más" }}</p>
      <i class="fa-solid fa-angle-up transform origin-center transition duration-200 ease-out ml-5"
        :class="{ '!rotate-180': cardDetails }"></i>
    </div>
