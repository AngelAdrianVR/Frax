<template>
    ...
    <div class="px-3 py-3">
      <div class="flex justify-between">
        ...
        <div class="flex items-center space-x-1 self-start">
          ...
          <i @click="$inertia.get(route('favorite-guests.edit', favoriteGuest.id))"
            class="fa-solid fa-pencil p-2 rounded-full bg-[#F2F2F2] cursor-pointer"></i>
        </div>
      </div>
      <!-- -------------- info ------- -->
      ...
      <div class="mt-4 grid grid-cols-2">
        <section>
          ...
          <div v-if="favoriteGuest.vehicle_details?.length > 0" @click="showVehicles = !showVehicles"
            ...
            <i class="fa-solid fa-angle-up transform origin-center transition duration-200 ease-out ml-5"
              :class="{ '!rotate-180': showVehicles }"></i>
          </div>
        </section>
        <section>
        ...
        <!-- --------- info vehículos ----------- -->
        <div v-for="(vehicle, index) in favoriteGuest.vehicle_details" :key="index" v-if="showVehicles"
          ...
          <section v-if="vehicleDetails[index]">
            ...
            <figure v-if="favoriteGuest.media_vehicle?.length > 0" class="w-48 h-32 rounded-xl mx-auto">
              <img class="object-contain bg-no-repeat h-32 rounded-lg"
                :src="favoriteGuest.media_vehicle[index]?.original_url" alt="Sin imagen" />
            </figure>
  ...
  <Modal :show="ShowGuestFormModal" @close="ShowGuestFormModal = false">
    <div class="mx-7 my-4 space-y-4 relative">
      ...
      <section class="lg:grid grid-cols-2">
        ...
        <div>
          <figure v-if="favoriteGuest.media_guest?.length > 0" class="bg-[#F2F2F2] w-44 h-36 rounded-sm">
            <img class="object-contain bg-no-repeat w-44 h-36" :src="favoriteGuest?.media_guest[0]?.original_url"
              alt="" />
          </figure>
          <div class="bg-[#F2F2F2] w-44 h-36 rounded-sm flex justify-center items-center" v-else><i
              class="fa-regular fa-user text-4xl text-gray3"></i></div>
          ...
          <div class="mt-7">
            ...
            <el-date-picker v-model="form.visit_date" type="date" placeholder="Seleccione"
        ...
        <div>
          <div>
            ...
            <el-select class="w-full" v-model="form.guest_type" clearable placeholder="Seleccione"
          ...
          <div v-if="form.guest_type == 'Vehicular'" class="border border-[#D9D9D9] rounded-sm mt-2 py-3 px-5">
            ...
            <p v-if="!favoriteGuest?.vehicle_details" class="text-xs text-red-600">Tu visita frecuente no tiene vehículos
              registrados*</p>
            <!-- --------- info vehículos ----------- -->
      ...
      <div class="flex justify-end space-x-1 pt-5 pb-1">
        ...
        <PrimaryButton :disabled="form.guest_type == 'Vehicular' && !favoriteGuest?.vehicle_details" @click="store">
