<template>
  <AppLayout title="Registrar visita">
    <div class="lg:py-7 lg:px-10 py-2 px-1">
      <Back />
      <form @submit.prevent="store" class="lg:mx-8 mx-2 mt-9 md:grid grid-cols-2 md:gap-9 md:p-4">
        <div>
          <div class="flex justify-center items-start space-x-12 text-sm mb-8">
            <el-radio-group v-model="guestForm.type" class="ml-4">
              <el-radio label="Visita" size="large">
                <i class="fa-regular fa-user mr-2"></i>
                Programar visita
              </el-radio>
              <el-radio label="Evento" size="large">
          ...
          <section v-if="guestForm.type == 'Visita'">
            <div class="relative">
              ...
              <p @click="guestForm.reset(); favoriteGuestModal = true; guestForm.qr_code = eventForm.qr_code"
            ...
            <div class="mt-4">
              ...
              <InputFilePreview @imagen="guestForm.guest_image = $event" @cleared="guestForm.guest_image = null" />
            </div>
          ...
          <div class="rounded-md border border-[#D9D9D9] mt-3" v-if="guestForm.guest_type == 'Vehicular'">
            ...
            <div class="pt-1 pb-5 px-7">
              <div>
                ...
                <InputFilePreview @imagen="guestForm.vehicle_image = $event" @cleared="guestForm.vehicle_image = null" />
              </div>
        ...
        <section v-if="selectedFavoriteGuest" class="lg:grid grid-cols-2">
          ...
          <div>
            <figure v-if="selectedFavoriteGuest?.media_guest?.length > 0" class="bg-[#F2F2F2] w-44 h-36 rounded-sm">
              <img class="object-contain bg-no-repeat w-44 h-36"
                :src="selectedFavoriteGuest?.media_guest[0]?.original_url" alt="" />
            </figure>
          ...
          <div>
            ...
            <div v-if="guestForm.guest_type == 'Vehicular'" class="border border-[#D9D9D9] rounded-sm mt-2 py-3 px-5">
              ...
              <p v-if="!selectedFavoriteGuest?.vehicle_details" class="text-xs text-red-600">Tu visita frecuente no tiene
                vehículos registrados*</p>
              <!-- --------- info vehículos ----------- -->
...
export default {
  ...
  methods: {
      ...
      if (this.guestForm.type == 'Visita') {
        ...
        this.eventForm.post(route("events.store"), {
          onSuccess: () => {
            this.$notify({
              title: "Correcto",
              message: "Se ha programado el evento",
              type: "success",
            });
          },
        });
    ...
    findFavoriteGuest() {
      ...
      this.guestForm.qr_code = this.eventForm.qr_code //guarda el qr generado porque al seleccionar favorito se resetea 
